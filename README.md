...
`tools/list` at `https://gateway.pipeworx.io/cocktails/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_cocktails \
  -H 'Content-Type: application/json' \
  -d '{"query":"margarita"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_cocktails`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
